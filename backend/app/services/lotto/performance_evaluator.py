...
def evaluate_single_draw(draw_no: int, ai_weights: dict = None, draws: List[Dict] = None) -> Dict:
    """
    단일 회차에 대한 성능 평가
    ...
    Args:
        ...
        draws: 전체 회차 데이터 (제공하지 않으면 내부에서 로드)

    Returns:
        평가 결과 딕셔너리
    """
    # draws가 제공되지 않으면 빈 리스트 처리
    if draws is None:
        print(f"⚠️ draws 데이터가 제공되지 않았습니다.")
        return None

    # 1. 당첨 번호 조회
    draw = None
    for d in draws:
        if d['draw_no'] == draw_no:
            draw = d
            break
    ...
    past_draws = [d for d in draws if d['draw_no'] < draw_no]

    if len(past_draws) < 10:
        print(f"⚠️ {draw_no}회 평가에 필요한 데이터가 부족합니다 (최소 10회 필요)")
        return None

    draws_dict = past_draws
...
def evaluate_latest_draw(draws: List[Dict]) -> Dict:
    """
    가장 최근 회차에 대한 성능 평가
    ...
    evaluation_result = evaluate_single_draw(draw_no, draws=draws)
...
def backtest_multiple_draws(draws: List[Dict], start_draw: int, end_draw: int) -> List[Dict]:
    """
    여러 회차에 대한 백테스팅
